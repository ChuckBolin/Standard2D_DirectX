...
void CEffect::Add(int index, float x, float y, float scale){
  //CLog *pLog = CLog::Instance();
...
void CEffect::Update(double timeDifference){
  ...
  for(int i = 0; i < m_effect.size(); ++i){
    ...
    m_effect[i].Time = (timeGetTime() - timeDifference) * 0.005f;  //adjust value to speed up effect
...
int CEffect::GetEmitterCount(int index){
  if(index < 0 || index > m_effect.size() - 1)
    return 0;
